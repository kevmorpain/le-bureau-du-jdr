...
- **Décision** : la préférence vit **sur `character_sheets`** ; `users` porte des **défauts** qui s'appliquent aux fiches n'ayant pas tranché. Résolution par **héritage live** — `fiche ?? compte ?? défaut codé` — et **non** par copie à la création. Conséquences directes :
  ...
  - résolution = **fonction pure partagée** serveur + client ([D10](#d10)), exposée **déjà résolue** par le GET fiche pour que le client ne ré-implémente pas la fusion. Le GET sélectionne déjà l'owner réduit à `{id, name}` : l'élargir à `{id, name, preferences}` est une ligne (⚠️ colonne neuve → passer par `srcSchema`, cf. le gotcha hub:db de `CLAUDE.md`).
- **Pourquoi** :
